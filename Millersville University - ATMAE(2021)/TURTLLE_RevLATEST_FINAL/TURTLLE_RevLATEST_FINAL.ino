...
void turnLeft() {
  for (int x = 0 ; x < 155 ; x++) { //90deg turn
...
void getRed() {
  ...
  moveInches(3.25, up); // second number: 1 is up, 0 is down
  delay(1000);
  TLVal = analogRead(TLSharpIR);
  ...
  moveInches(0.25, up); // second number: 1 is up, 0 is down
  delay(1000);
  TLVal = analogRead(TLSharpIR);
  while (TLVal > 300) {
  TLVal = analogRead(TLSharpIR);
  ...
  delay(1000);
  moveInches(3.5, down); // second number: 1 is up, 0 is down
  delay(1000);
