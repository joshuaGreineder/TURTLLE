...
void clearWall() {
  for (int x = 0 ; x < 48  ; x++) { //move past wall
    servoMove(leftServo, left_forward_slow);
    servoMove(rightServo, right_forward_slow);
    CheckForDelay();
    delay(20); // This value changes speed of motor, do not set < 20ms
...
void turnLeft() {
  for (int x = 0 ; x < 175 ; x++) { //90deg turn
    servoMove(leftServo, left_backward_slow);
    servoMove(rightServo, right_forward_slow);
    CheckForDelay();
    delay(20); // This value changes speed of motor, do not set < 20ms
...
void getRed() {
  ...
  moveInches(5.75, up); // second number: 1 is up, 0 is down
  delay(1000);
  ...
  for (int x = 0 ; x < 35 ; x++) { //move away the crate
    servoMove(leftServo, left_backward_slow);
    servoMove(rightServo, right_backward_slow);
    delay(20); // This value changes speed of motor, do not set < 20ms
  }
